...
generate_entry() {
    user=$(( ( RANDOM % $num_users )  + 1 ))
    clip=$(( ( RANDOM % $num_clips )  + 1 ))
    rating=$(( ( RANDOM % 10 )  + 1 ))
    entry="($user,$clip,$rating)"
}
#-------------------------------------------------------------------------------
...
sleep 5
./bin/kafka-topics.sh --create --zookeeper localhost:2181 --replication-factor 1 --partitions 1 --topic stream
...
while [[ "$keep_sending" = "1" ]]; do
  ...
  i=$((i+1))
  sleep 1
done | ./bin/kafka-console-producer.sh --broker-list localhost:9092 --topic stream --property "parse.key=true" --property "key.separator=:" >/dev/null &
kv_pid=$!
